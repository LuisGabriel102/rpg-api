    ...
    with ui.column().classes("w-full gap-4"):
        ...
        with ui.card().classes("w-full bg-zinc-800 border border-zinc-700 p-4 gap-3"):
            ...
            if not tem_ancora:
                ...
                # Handler do botão Upload — abre dialog com selector de arquivo
        ...
        with ui.row().classes("w-full justify-center gap-1 flex-wrap"):
            ...
            if img.get("prompt_usado"):
                async def _copiar_prompt(prompt=img["prompt_usado"]):
                    ...
                    safe = (prompt or "").replace("\\", "\\\\").replace("`", "\\`")
...
def _abrir_dialog_upload(
    ...
    A imagem sobe pro R2 + salva no banco como 'rascunho'. Usuário depois
    ...
    with ui.dialog() as dialog, ui.card().classes(
        ...
        ui.label(
            "Envia uma imagem do teu computador (JPG, PNG ou WEBP, até 5MB). "
            "Ela entra na galeria como 'rascunho' — tu decide depois se promove."
        ).classes("text-zinc-400 text-sm")

        rotulo_input = (
            ui.input(
                label="Rótulo narrativo (opcional)",
                placeholder="ex: referência de Pinterest, edição manual",
            )
            .props("outlined dense color=amber-8 dark")
            .classes("w-full")
        )
        ...
        async def _on_upload(e):
            """Callback do ui.upload — dispara quando arquivo é selecionado.

            Usa EXATAMENTE o mesmo padrão que oficina_npcs_42.py (que já funciona
            em produção no projeto). Evita inventar acesso a API de upload.
            """
            # Padrão do projeto: e.file + await file.read()
            file = e.file
            file_bytes = await file.read()

            # Detectar content_type (múltiplos atributos possíveis)
            content_type = (
            ...
            # Fallback pelo nome do arquivo se nenhum atributo retornar
            if not content_type:
                fname = (
            ...
            try:
                ...
                # 4) Recarrega galeria + comparação
                estado.imagens = await listar_imagens_npc(npc_id)
                estado.canonica = await buscar_canonica_atual(npc_id)
