...
def formatar_contexto_para_prompt(ctx: dict, incluir_ultimos_instantes: bool = True) -> str:
    """
    ...
    incluir_ultimos_instantes (A2.2 / CAMINHO A): quando False, OMITE a seção
    "## Os últimos instantes" (tier M3, turnos_recentes). Essa prosa é duplicata
    byte-idêntica do histórico de mensagens completo (estado.historico), então
    cortá-la do <contexto> não perde informação. Os demais tiers (dívidas, recap,
    fatos, boatos) ficam intocados. Default True = byte-idêntico ao de hoje.
    """
    blocos = []
    ...
    # turnos recentes (curto prazo). A2.2: omitido quando incluir_ultimos_instantes=False
    # (duplicata do historico de mensagens; os outros tiers seguem intactos).
    turnos = ctx.get("turnos_recentes") or []
    if turnos and incluir_ultimos_instantes:
        falas = [
