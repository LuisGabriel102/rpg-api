...
# Conserto LOCAL: pula a Basic Auth do /jogar e /jogar-c quando ligado. Default OFF
# (env ausente) -> auth normal, producao SEGUE protegida. Liga-se SO no ambiente local
# (run_local.py / .env local), NUNCA no Railway. Fail-safe: so o valor explicito on liga.
AUTH_OFF_JOGAR: bool = os.getenv("OFICINA_AUTH_OFF", "").strip().lower() in ("1", "true", "yes", "on")