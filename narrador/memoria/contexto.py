...
async def carregar_contexto(
    ...
    if not ctx:
        return ""
    # A2.2-flip: corta "Os últimos instantes" (duplicata byte-idêntica do historico de
    # mensagens). Lossless e -33,8% de input/sessao (medido). Demais tiers intactos.
    return formatar_contexto_para_prompt(ctx, incluir_ultimos_instantes=False)
