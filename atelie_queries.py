...
async def carregar_npc_completo(npc_id: int) -> Optional[dict]:
    ...
    try:
        row = await conn.fetchrow(
            """
            SELECT id, nome, nome_curto, camada, raca, idade, profissao,
                   facoes,
...
async def inserir_imagem_upload(
    npc_id: int,
    url: str,
    rotulo_narrativo: Optional[str] = None,
) -> int:
    """Persiste metadados de uma imagem enviada via upload (não gerada por IA).

    A imagem já foi subida ao R2 pelo caller. Esta função só registra no banco
    como status='rascunho' (usuário decide depois se promove).
...
async def deletar_imagem(imagem_id: int) -> Optional[str]:
    ...
    try:
        row = await conn.fetchrow(
            "SELECT url, r2_key FROM npc_imagens WHERE id = $1",
            imagem_id,
        )
        if not row:
            return None
        await conn.execute("DELETE FROM npc_imagens WHERE id = $1", imagem_id)
        # Retorna URL pra caller deletar do R2
        return row["url"]
    finally:
        await conn.close()
